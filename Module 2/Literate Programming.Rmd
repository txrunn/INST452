...
inTrain <- createDataPartition(fd$food_desert, p = .8, list = FALSE, times = 1)
...
# model
logit <- train(food_desert ~ ., data = fd_train, method = "glm", trControl = ctrl)
...
perc_white <- fd %>% 
        ...
        summarise(
          race_perc_white = mean(race_perc_white, na.rm = TRUE)
        )
...
house_perc_own <- fd %>% 
        group_by(food_desert) %>%
        summarise(
          house_perc_own = mean(house_perc_own, na.rm = TRUE)
        )
...
SNAP_perc <- fd %>% 
        group_by(food_desert) %>%
        summarise(
          SNAP_perc = mean(SNAP_perc, na.rm = TRUE)
        )
